...
# ===========================================================================
# Google helpers (shared by Gmail + Calendar)
# ===========================================================================
def _resolve_token_path(account: str | None):
    """Map an account alias (or email) to its token Path, or an error string."""
    alias = (account or config.DEFAULT_GOOGLE_ACCOUNT).strip().lower()
    alias = config.GOOGLE_EMAIL_TO_ALIAS.get(alias, alias)
    token_path = config.GOOGLE_ACCOUNTS.get(alias)
...
def _google_service(api: str, version: str, account: str | None = None):
    """Build an authenticated Google API service, or return an error string.

    Loads the token saved by ``credentials/google_auth.py`` for the requested
    ``account`` (default: the "personal" account), refreshing it in place when
    expired. Returns the service object on success, otherwise a human-readable
    ``error: ...`` string the caller can return directly.
    """
    token_path = _resolve_token_path(account)
    if isinstance(token_path, str):
        return token_path
    if not token_path.exists():
        alias = config.GOOGLE_EMAIL_TO_ALIAS.get(
            (account or config.DEFAULT_GOOGLE_ACCOUNT).strip().lower(),
            (account or config.DEFAULT_GOOGLE_ACCOUNT).strip().lower(),
        )
        return f"error: account '{alias}' not authorized; run python credentials/google_auth.py {alias}"
    ...
    try:
        ...
        return f"error: google libraries not installed ({exc})"
    ...
    try:
        ...
        return f"error: invalid token file ({exc}); re-run google_auth.py"
    ...
    if not creds.valid:
        if creds.expired and creds.refresh_token:
            try:
                ...
                return f"error: token refresh failed ({exc}); re-run google_auth.py"
        else:
            return "error: token invalid; re-run python credentials/google_auth.py"
...
def _header(headers: list, name: str) -> str:
    """Return a named header value from a Gmail message header list."""
    for h in headers:
...
# ===========================================================================
@mcp.tool()
def gmail_read(max_results: int = 10, query: str = "", account: str = "personal") -> str:
    """List recent emails (Gmail search syntax). Empty query → unread inbox.

    ``account`` selects which mailbox: "personal" (personal email, default) or
    "work" (work email). Returns one line per message: id, from,
    subject, date, snippet.
    """
    service = _google_service("gmail", "v1", account)
    ...
    if not messages:
        return f"(no messages for query: {effective_query})"
    ...
    for m in messages:
        ...
        lines.append(
            f"[{m['id']}] {sender} | {subject} | {date}\n    {snippet}"
        )
    return "\n".join(lines)


@mcp.tool()
def gmail_read_full(message_id: str, account: str = "personal") -> str:
    """Return the plaintext body of a specific email (truncated to 8000 chars).

    ``account``: "personal" (default) or "work". Must match the mailbox the
    ``message_id`` came from.
    """
    service = _google_service("gmail", "v1", account)
    if isinstance(service, str):
        return service
    ...
    def extract(payload: dict) -> str:
        """Depth-first search for a text/plain (or text/html) body part."""
        ...
        for part in payload.get("parts", []) or []:
            ...
            if text:
                return text
        # Fallback: any html body if no plaintext was found.
        if mime == "text/html" and data:
            return base64.urlsafe_b64decode(data).decode("utf-8", "replace")
        return ""

    body = extract(msg.get("payload", {})) or msg.get("snippet", "")
    if len(body) > 8000:
        body = body[:8000] + "\n...[truncated]"
    return body
...
@mcp.tool()
def gmail_send(to: str, subject: str, body: str, account: str = "personal") -> str:
    """Send an email from the chosen account. Returns 'sent to {to}'.

    ``account``: "personal" (personal email, default) or "work"
    (work email) — this is the *From* mailbox.

    The confirmation rule lives in CLAUDE.md — this tool just executes; the
    agent is responsible for asking the user before calling it.
    """
    service = _google_service("gmail", "v1", account)
    if isinstance(service, str):
        return service
...
# Google Drive tools
# ===========================================================================
# Native Google editor files can't be downloaded raw — they must be exported to
# a concrete format. Map each Google MIME type to a sensible text export.
GOOGLE_DOC_EXPORTS = {
...
@mcp.tool()
def drive_list(query: str = "", max_results: int = 20, account: str = "personal") -> str:
    """List Drive files (most-recently-modified first), one per line.

    ``query`` is Drive query syntax, e.g. ``name contains 'budget'`` or
    ``mimeType = 'application/vnd.google-apps.document'``. Empty → all files.
    ``account``: "personal" (default) or "work". Returns: id, name, mimeType, modified.
    """
    service = _google_service("drive", "v3", account)
    if isinstance(service, str):
        return service
    try:
        resp = (
    ...
    if not files:
        return f"(no files for query: {query or 'all'})"
    lines = []
    for f in files:
        size = f.get("size")
        size_str = f" | {int(size) / 1024:.1f} KB" if size else ""
        lines.append(
            f"[{f['id']}] {f.get('name', '?')} | {f.get('mimeType', '')} "
            f"| {f.get('modifiedTime', '')}{size_str}"
        )
    return "\n".join(lines)
...
def drive_read(file_id: str, account: str = "personal") -> str:
    """Return the text content of a Drive file (truncated to 8000 chars).

    Google Docs/Sheets/Slides are exported to text/CSV; other files are read as
    UTF-8. ``account``: "personal" (default) or "work".
    """
    service = _google_service("drive", "v3", account)
    if isinstance(service, str):
        return service
    try:
        ...
        if mime in GOOGLE_DOC_EXPORTS:
            data = (
                service.files()
                .export(fileId=file_id, mimeType=GOOGLE_DOC_EXPORTS[mime])
                .execute()
            )
        else:
...
def drive_create(
    ...
    """Create a new Drive file with ``content``. Returns 'created: {name} ({id})'.

    Set ``as_google_doc=True`` to convert the uploaded text into a native Google
    Doc. ``account``: "personal" (default) or "work".
    """
    service = _google_service("drive", "v3", account)
    if isinstance(service, str):
        return service
    ...
    try:
        ...
        return f"created: {f.get('name')} ({f.get('id')})"
    except Exception as exc:  # noqa: BLE001
        return f"error: {exc}"
...
def drive_update(
    ...
    """Overwrite an existing Drive file's content. Returns 'updated: {name} ({id})'.

    For native Google Docs, pass plain text (mime_type='text/plain') and Drive
    re-imports it. ``account``: "personal" (default) or "work".
    """
    service = _google_service("drive", "v3", account)
    if isinstance(service, str):
        return service
    ...
    try:
        ...
        return f"updated: {f.get('name')} ({f.get('id')})"
    except Exception as exc:  # noqa: BLE001
        return f"error: {exc}"
...
# ===========================================================================
@mcp.tool()
def calendar_list_events(days_ahead: int = 7, account: str = "personal") -> str:
    """List primary-calendar events for the next N days, one per line.

    ``account``: "personal" (default) or "work".
    """
    service = _google_service("calendar", "v3", account)
    if isinstance(service, str):
        return service
...
def calendar_create_event(
    ...
    ``start``/``end`` use ``"YYYY-MM-DD HH:MM"`` in the system local timezone.
    ``account``: "personal" (default) or "work".
    Returns ``created: {title} on {start}`` or an ``error: ...`` string.
    """
    service = _google_service("calendar", "v3", account)
    if isinstance(service, str):
        return service

    # Resolve the system local timezone for RFC3339 offsets.
    local_tz = datetime.now().astimezone().tzinfo
    try:
        start_dt = datetime.strptime(start, "%Y-%m-%d %H:%M").replace(
            tzinfo=local_tz
        )
        end_dt = datetime.strptime(end, "%Y-%m-%d %H:%M").replace(
            tzinfo=local_tz
        )
    except ValueError:
        return "error: start/end must be in format 'YYYY-MM-DD HH:MM'"
    ...
    try:
        ...
        return f"created: {title} on {start}"
    except Exception as exc:  # noqa: BLE001
        return f"error: {exc}"
